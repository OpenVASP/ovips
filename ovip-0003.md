...
The Index Contract implements the Addressing Layer as described in [ovip-0009](). There can be multiple implementations of this layer and even multiple istances of the Index Contract. To ensure global uniqueness the **VASP Code Type** specifies for which implementation of the Addressing Layer the VASP Code is registered. The combination of the VASP Code Type and the VASP Code globally, uniquely identify a VASP and is referred to as the **VASP Identifier**
...
Each VASP participating in the OpenVASP protocol deploys and owns a dedicated VASP Contract. It is used to provide the public keys required to decrypt and verify protocol messages that were signed and sent by the VASP. In addition, the VASP Contract can store pointers to registries, where credentials associated with 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA2MzM4NTM1MiwxMzUxNjg4OTQ5XX0=
-->