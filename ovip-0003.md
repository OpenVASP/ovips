...
The VASP Contract is used to provide the public keys required to decrypt and verify protocol messages that were signed and sent by the VASP. In addition, the VASP Contract can store pointers to registries, where credentials associated with that VASP Identifier can be found.
...
```pseudocode
addRegistry(address): bool
```

**Description:**  
Adds an address to `newKey`.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDI5OTU2NTEwLDE5MTY2NzY0MDYsLTU3Nj
E3NDQ1LDEzNTE2ODg5NDldfQ==
-->