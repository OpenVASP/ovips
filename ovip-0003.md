...
TransportKeyChanged(bytes4 indexed vaspCode, string previousTransportKey, string newTransportKey);
MessageKeyChanged(bytes4 indexed vaspCode, string previousMessageKey, string newMessageKey);
SigningKeyChanged(bytes4 indexed vaspCode, string previousSigningKey, string newSigningKey);
```
...
**NB!**
The *signingKey*, *messageKey* and the *transportKey* must be stored in [compressed](https://learnmeabitcoin.com/guide/public-key) form. That is it must be represented with exactly 33 bytes (66 characters) and is prefixed with '02' or '03'.
