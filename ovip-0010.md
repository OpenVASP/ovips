...
1. Iff the `isCLOSE` flag on the respective Outbound Envelope Queue item is `True`, then delete the respective *Connection* item from the *Connection* set.
2. Delete the respective item from the Outbound Envelope Queue.
3. Iff the `isCLOSE` flag on the respective Outbound Envelope Queue item is `False`, then notify the session layer's *Exception Handling* (see OVIP-7, 3.1) about the interruption.
