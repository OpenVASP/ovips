...
**Connection**</br>
Two *Nodes* sharing a common identifier representing a connection and an associated shared key, which can be used to exchange *Envelopes* containing an encrypted *Message*. Both *Nodes* have sent each other a Whisper topic to be used to send messages related to this connection.
...
| ----- | -------------------------- | -------------------------------------------------- | ------- |
| `001` | *Active Node*              | INVITE - Inviting for a *Connection*               | 6.1     |
| `010` | *Passive Node*             | ACCEPT Accepting a *Connection* invitation         | 6.2     |
| `011` | *Passive Node*             | DENY - Denying a *Connection* invitation           | 6.3     |
| `100` | *Active* or *Passive Node* | UPDATE - Sending an *Envelope* over a *Connection* | 6.4     |
| `101` | *Active* or *Passive Node* | CLOSE - Closing a *Connection*                     | 6.5     |
| `000` | *Active* or *Passive Node* | ACK - Acknowledge *Envelope* receipt               | 6.6     |

## 5. Connections

This sections explains how the *Node* is handling *Connections* without being normative about the actual implementation.
...
Relevant *Connection* properties are listed in the table below.

| Element        | Description                                                  |
| :------------- | :----------------------------------------------------------- |
| `connection`   | *Connection* identifier                                      |
| `topic`        | Topic used to identify incoming *Envelopes* related to this *Connection* |
| `key`          | Shared symmetric key to encrypt/decrypt *Envelopes* sent/received over this *Connection* |
| `symmetric`    | Flag indicating whether encryption is symmetric or asymmetric |
| `lastEnvelope` | Last *Envelope* sent over this *Connection*                  |
| `ackd`         | Flag indicating whether `lastEnvelope` has been acknowledged by the other *Node* |
| `expiry`       | Time when the waiting period for the acknowledgement of `lastEnvelope` ends and it is resent |
| `resends`      | Number of times `lastEnvelope` had to be resent              |

### 5.3. Permanent Connection

Each VASP using the OpenVASP protocol must be contactable via its VASP Identifier. The VASP's *Node* must therefore always have this *Connection* in its set, which is called the Permanent *Connection*. It has the following properties:

| Element        | Value                                                        |
| :------------- | :----------------------------------------------------------- |
| `connection`   | VASP Identifier                                              |
| `topic`        | VASP Identifier                                              |
| `key`          | Shared symmetric key to encrypt/decrypt *Envelopes* sent/received over this *Connection* |
| `symmetric`    | Flag indicating whether encryption is symmetric or asymmetric |
| `lastEnvelope` | Last *Envelope* sent over this *Connection*                  |
| `ackd`         | Flag indicating whether `lastEnvelope` has been acknowledged by the other *Node* |
...
8. Add item to the set of *Connections*:</br>
   ...
   | `topic`        | `topicA`                            |
   | `key`          | `sharedKey`                         |
   | `lastEnvelope` | *Envelope* identifier set in step 6 |
   | `ackd`         | `False`                             |
...
2. Send *Envelope* created in step 7.
