...
## 5. Implementing the Session Layer
...
### 5.1. Initiating a Session (INVITE)

1. Input: Session Request Message JSON as specified in OVIP-7, section 4.1 (`sessionMessage`) containing the `receiver`.

2. Retrieve public `transportKey` available from the `receiver`'s' VASP Contract.

3. Generate random `connectionIdentifier`.

4. Generate random `topicA` the *Active Node* will listen for to identify incoming *Envelope* sent over the *Connection*.
...
6. Derive `sharedKey` based on the public `transportKey` and `ecdhPublic` generated in the previous step.
...
9. Create the *Envelope* out of the elements defined in 3. and the specified values in the table below.</br>

   | Element | Value / Specification                                        |
   | :------ | :----------------------------------------------------------- |
   | `Topic` | `receiver`'s VASP Identifier                                 |
   | `Data`  | Asymmetrically encrypted OpenVASP Payload (see step 8) using SECP-256k1 with the public `transportKey` |

   </br>

9. Make the *Node* listen to incoming *Envelopes* with topic `topicA` and associated `sharedKey`.

10. Send the Envelope created in step 8.
